...
exports.mostrarPedxplas = async (req, res, next) => {
    try {
        const pedxplas = await Pedxplas.find({}).populate('_id').populate('id_plato');
        res.json(pedxplas);
    } catch (error) {
        console.log(error);
        next();
...
exports.mostrarPedxpla = async (req, res, next) => {
    const pedxpla = await Pedxplas.findById(req.params.idPedxpla).populate('_id').populate('id_plato',{_id:0});
