...
exports.mostrarIngredientes = async (req, res, next) => {
    try {
        const ingredientes = await Ingredientes.find({}).populate('id_ingrediente',{_id:0})
                                                        .populate('id_bebida',{_id:0})
                                                        .populate('id_plato',{_id:0});
...
exports.mostrarIngrediente = async (req, res, next) => {
    const ingrediente = await Ingredientes.findById(req.params.idIngrediente).populate('id_ingrediente',{_id:0})
                                                                             .populate('id_bebida',{_id:0})
                                                                             .populate('id_plato',{_id:0});
